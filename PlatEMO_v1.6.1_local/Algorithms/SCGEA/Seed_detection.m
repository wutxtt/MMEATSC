function [Population, Seed, Species, Group] = Seed_detection(Population,Species,N,mod, Global,center, Nns)
...
%                     
        ...
        for i = 1:length(Front)
            range=max(Front(i).p.decs,[],1)-min(Front(i).p.decs,[],1);
            r=(range)*0.1; %取阈值r
            G = NCM(Front(i).p.decs, r);
            for j = 1:length(G)
                Group(k+j).p = Front(i).p(G(j).p);
            end
            k = k+j;
        end
%         Seed = [];
