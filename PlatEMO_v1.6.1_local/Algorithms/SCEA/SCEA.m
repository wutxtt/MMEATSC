...
%--------------------------------------------------------------------------
    ...
    Nns = Global.ParameterSet(Global.N/80);
    ...
    ZDmin = min(Population.decs,[],1);
%     [~,FrontNo,CrowdDis] = EnvironmentalSelection(Population,Global.N);

    %% Optimization
    while Global.NotTermination(Population)
%         MatingPool = TournamentSelection(2,Global.N,FrontNo,-CrowdDis);
        if Global.evaluated < 0.05*Global.evaluation
            mod = 1;
        else
            mod = 2;
        end
        
        [Population,Species,center, r_dec] = Species_division(Population,Nns,mod, Global);
        [Population, Seed, Species, Front] = Seed_detection(Population,Species,Global.N,mod, Global,center, r_dec);
        if mod == 1
            Offspring = [];
            Zmin = min(Population.objs,[],1);
            ZDmin = min(Population.decs,[],1);
...
%                 MatingPool = TournamentSelection(2,length(Front(i).p),FrontNo);
                allFront = [];
                CrowdDis = Crowding(Front(i).p.decs);
...
%                 MatingPool = randi(length(allFront),1,length(Front(i).p));
                MatingPool = TournamentSelection(2,length(allFront),-CrowdDis);
...
%                 size_offspring = size_offspring + floor(Global.N/length(Front));
        ...
        Population = Seed_conservation([Population,Offspring],Seed,Species,Global.N,mod,Front,Global,Z,ZD,Zmin,ZDmin,Front_Zmin,Front_ZDmin,center);
    end
end