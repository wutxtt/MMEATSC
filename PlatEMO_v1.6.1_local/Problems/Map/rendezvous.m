function varargout = rendezvous(Operation,Global,input)
% <problem> <MMPPO>
...
%--------------------------------------------------------------------------
    ...
    switch Operation
        case 'init'
            Global.M        = 2;
            Global.D        = 2;% x1=tw x2=tf
            Global.lower    = [0,0.03]; %hours
            Global.upper    = [10.8,10.8];
...
%                 oev1(4) = dtr * 0;
                oev1(5) = dtr * 0;
                oev1(6) = dtr * 96.036 * PopDec(i,1)+pi;
                oev2 = zeros(6,1);
                oev2(1) = 26553.071184;
                oev2(2) = 0.667;
                oev2(3) = dtr * 63.4;
...
%                 oev2(4) = dtr * 0;
%                 oev2(5) = dtr * 0;
                oev2(6) = dtr * 12.024 * (PopDec(i,1)+PopDec(i,2))+2*pi;
                mu=398600.4415;
                ...
                ndvi = norm(dvi);
                ndvf = norm(dvf);
                PopObj(i,1) = (norm(dvi) + norm(dvf))/1000;
                PopObj(i,2) = PopDec(i,2)+PopDec(i,1);
            end
            PopCon = [];
